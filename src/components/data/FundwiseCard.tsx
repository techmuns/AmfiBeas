...
export function FundwiseCard({ matrix }: { matrix: FundwiseMatrix }) {
  ...
  const csvRows: FundwiseCsvRow[] = useMemo(() => {
    const exportRows = [
      ...matrix.rows,
      ...(matrix.othersRow ? [matrix.othersRow] : []),
      ...(matrix.totalRow ? [matrix.totalRow] : []),
    ];
      ...
      subtitleNode={
        <div className="space-y-0.5">
          <p className="text-xs text-muted-foreground">
            Each AMC&rsquo;s share of total industry AAUM by quarter, with the
            QoQ move in basis points — read down a column for the pecking
            order, across a row for momentum. Remaining Others aggregates the
            tail so shares sum to 100%; Total Market is the industry total.
          </p>
          <p className="text-[11px] text-muted-foreground/80">
            {`Top ${matrix.rows.length} AMCs by AAUM${matrix.othersRow ? ` + ${matrix.othersRow.displayName}` : ""} · ${matrix.quarterLabels[0]} → ${matrix.quarterLabels[latestIdx]}`}
          </p>
        </div>
      }
      action={
    ...
    >
      {leaders && (
        <KeyTakeaway
          ...
          detail={
            <>
              Top-5 AMCs hold {leaders.top5.toFixed(1)}% of total market AAUM.{" "}
              {metric === "share"
