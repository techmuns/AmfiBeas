 ...
 * scripts/build-cap-flows.ts from the AMC-direct equity holdings
 * (public/holdings-direct) — the same filings behind the MFs Portfolio Tracker.
 */

export interface CapFlowRow {
  company: string;
  /** Security identifier from the AMC filing (ISIN) — the join key into the
   *  shares-outstanding feed; not displayed. */
  fincode: string;
  netCr: number;
